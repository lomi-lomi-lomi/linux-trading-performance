...
Same with CPUs: accessing memory on YOUR socket = ~100ns. Accessing memory on the OTHER socket = 150-300ns (2-3x slower!). That slowdown is called the "NUMA penalty."
...
Bonus tip: If you can't disable HT system-wide, you can isolate sibling CPUs — find them with cat /sys/devices/system/cpu/cpu0/topology/thread_siblings_list and just leave the sibling idle!
...
💡 Why It Matters:
In trading, predictability is everything. Even a 10μs blip from a random background task can mess up your p99 latency. With isolcpus, you create "clean" CPUs that ONLY run your trading code. Coinbase literally says: "Prevent other user threads from taking CPU time."
...
💡 Why It Matters:
Coinbase specifically chose z1d instances for the NVMe storage. Their Raft consensus requires fast disk writes — without it, "everything will run to a slow crawl." The disk is often a hidden bottleneck that people overlook.
...
ENA = standard AWS network adapter (25-50µs)
ENA Express = same adapter + SRD protocol magic (10-25µs, better P99)
EFA = direct hardware access, kernel bypass (5-10µs, requires app changes)
